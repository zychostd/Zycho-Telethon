...
from Zycho import BOTLOG_CHATID
...
async def on_snip(event):
    """costums logic."""
    try:
        from Zycho.modules.sql_helper.snips_sql import get_snip
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"costum (\w*)"))
async def on_snip_save(event):
    """For .costum command, saves costums for future use."""
    try:
        from Zycho.modules.sql_helper.snips_sql import add_snip
...
@bot.on(Zycho_cmd(outgoing=True, pattern="costums$"))
async def on_snip_list(event):
    """For .costums command, lists costums saved by you."""
    try:
        from Zycho.modules.sql_helper.snips_sql import get_snips
    except AttributeError:
        await event.edit("**Berjalan pada mode Non-SQL!**")
        return
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"delcostum (\w*)"))
async def on_snip_delete(event):
    """For .delcostum command, deletes a costum."""
    try:
        from Zycho.modules.sql_helper.snips_sql import remove_snip
    except AttributeError:
        await event.edit("**Berjalan pada mode Non-SQL!**")
        return
