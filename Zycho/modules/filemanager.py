...
from Zycho.utils import humanbytes

MAX_MESSAGE_SIZE_LIMIT = 4095


@bot.on(Zycho_cmd(outgoing=True, pattern=r"ls(?: |$)(.*)"))
async def lst(event):
    if event.fwd_from:
        return
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"rm(?: |$)(.*)"))
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"rn ([^|]+)\|([^|]+)"))
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"zip (.*)"))
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"unzip (.*)"))
