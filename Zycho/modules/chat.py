...
from Zycho import ALIVE_NAME, BLACKLIST_CHAT, BOTLOG, BOTLOG_CHATID
from Zycho import CMD_HANDLER as cmd
from Zycho import CMD_HELP, bot
from Zycho.events import Zycho_cmd, register
from Zycho.modules.ping import absen
from Zycho.utils import edit_or_reply, get_user_from_event


@bot.on(Zycho_cmd(outgoing=True, pattern="userid$"))
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"link(?: |$)(.*)"))
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"bots(?: |$)(.*)"))
async def _(event):
    if event.fwd_from:
        return
...
@bot.on(Zycho_cmd(outgoing=True, pattern="kickme$"))
...
@bot.on(Zycho_cmd(outgoing=True, pattern="kikme$"))
...
@bot.on(Zycho_cmd(outgoing=True, pattern="unmutechat$"))
async def unmute_chat(unm_e):
    try:
        from Zycho.modules.sql_helper.keep_read_sql import unkread
...
@bot.on(Zycho_cmd(outgoing=True, pattern="mutechat$"))
async def mute_chat(mute_e):
    try:
        from Zycho.modules.sql_helper.keep_read_sql import kread
    except AttributeError:
...
@bot.on(Zycho_cmd(incoming=True))
async def keep_read(message):
    try:
        from Zycho.modules.sql_helper.keep_read_sql import is_kread
    except AttributeError:
        return
    kread = is_kread()
    if kread:
...
@bot.on(Zycho_cmd(outgoing=True, pattern="s/"))
async def sedNinja(event):
    try:
        from Zycho.modules.sql_helper.globals import gvarstatus
    except AttributeError:
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"regexninja (on|off)$"))
async def sedNinjaToggle(event):
    if event.pattern_match.group(1) == "on":
        try:
            from Zycho.modules.sql_helper.globals import addgvar
        except AttributeError:
            return await event.edit("**Running on Non-SQL mode!**")
        ...
        try:
            from Zycho.modules.sql_helper.globals import delgvar
        except AttributeError:
            return await event.edit("**Running on Non-SQL mode!**")
        delgvar("regexNinja")
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"chatinfo(?: |$)(.*)"))
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"invite(?: |$)(.*)"))
async def _(event):
    if event.fwd_from:
        return
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"inviteall ?(.*)"))
...
@bot.on(Zycho_cmd(outgoing=True, pattern="getmember$"))
...
@bot.on(Zycho_cmd(outgoing=True, pattern="addmember$"))
