...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"(d)?fban(?: |$)(.*)"))
async def fban(event):
    """Bans a user from connected federations."""
    try:
        from Zycho.modules.sql_helper.fban_sql import get_flist
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"unfban(?: |$)(.*)"))
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"addf(?: |$)(.*)"))
async def addf(event):
    """Adds current chat to connected federations."""
    try:
        from Zycho.modules.sql_helper.fban_sql import add_flist
    except IntegrityError:
        return await event.edit("**Running on Non-SQL mode!**")
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"delf$"))
async def delf(event):
    """Removes current chat from connected federations."""
    try:
        from Zycho.modules.sql_helper.fban_sql import del_flist
    except IntegrityError:
        return await event.edit("**Running on Non-SQL mode!**")
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"listf$"))
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"clearf$"))
async def clearf(event):
    """Removes all chats from connected federations."""
    try:
        from Zycho.modules.sql_helper.fban_sql import del_flist_all
    except IntegrityError:
        return await event.edit("**Running on Non-SQL mode!**")
