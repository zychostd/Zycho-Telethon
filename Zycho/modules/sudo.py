...
from Zycho import CMD_HELP, HEROKU_API_KEY, HEROKU_APP_NAME, SUDO_HANDLER, SUDO_USERS
...
@Zycho_cmd(pattern="addsudo(?:\s|$)([\s\S]*)")
async def add(event):
    suu = event.text[9:]
...
@Zycho_cmd(pattern="delsudo(?:\s|$)([\s\S]*)")
async def _(event):
    suu = event.text[8:]
