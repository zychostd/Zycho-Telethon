...
from Zycho.utils import edit_or_reply, load_module, remove_plugin, reply_id


@bot.on(Zycho_cmd(outgoing=True, pattern="install$"))
async def _(event):
    if event.fwd_from:
        return
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"psend ([\s\S]*)"))
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"uninstall (?P<shortname>\w+)"))
