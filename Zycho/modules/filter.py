...
from Zycho import BLACKLIST_CHAT, BOTLOG, BOTLOG_CHATID
...
async def filter_incoming_handler(handler):
    ...
    try:
        if not (await handler.get_sender()).bot:
            try:
                from Zycho.modules.sql_helper.filter_sql import get_filters
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"filter (.*)"))
async def add_new_filter(new_handler):
    ...
    try:
        from Zycho.modules.sql_helper.filter_sql import add_filter
    except AttributeError:
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"stop (.*)"))
async def remove_a_filter(r_handler):
    """For .stop command, allows you to remove a filter from a chat."""
    try:
        from Zycho.modules.sql_helper.filter_sql import remove_filter
    except AttributeError:
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"delfilterbot (.*)"))
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"filters$"))
