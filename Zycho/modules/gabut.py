...
from Zycho import ALIVE_NAME, StartTime, bot
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"keping$"))
async def _(pong):
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"kping$"))
async def _(pong):
    """For .ping command, ping the userbot from any chat."""
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"assal(?: |$)(.*)"))
async def _(event):
