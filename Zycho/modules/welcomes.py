...
from Zycho import BLACKLIST_CHAT, BOTLOG_CHATID, CLEAN_WELCOME
...
async def welcome_to_chat(event):
    try:
        from Zycho.modules.sql_helper.welcome_sql import (
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"setwelcome(?: |$)(.*)"))
async def save_welcome(event):
    if event.chat_id in BLACKLIST_CHAT:
        return await event.edit("**Perintah ini Dilarang digunakan di Group ini**")
    try:
        from Zycho.modules.sql_helper.welcome_sql import add_welcome_setting
    except AttributeError:
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"checkwelcome$"))
async def show_welcome(event):
    try:
        from Zycho.modules.sql_helper.welcome_sql import get_current_welcome_settings
    except AttributeError:
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"rmwelcome$"))
async def del_welcome(event):
    try:
        from Zycho.modules.sql_helper.welcome_sql import rm_welcome_setting
    except AttributeError:
        return await event.edit("`Running on Non-SQL mode!`")
