...
from Zycho.utils import edit_delete, edit_or_reply
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"stats$"))
async def stats(
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"(ustat|deteksi|ustats)(?: |$)(.*)"))
async def _(event):
    if event.fwd_from:
        return
