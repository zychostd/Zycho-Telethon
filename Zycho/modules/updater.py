...
from Zycho import CMD_HELP, HEROKU_API_KEY, HEROKU_APP_NAME, UPSTREAM_REPO_URL, bot
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            await event.edit(
                f"{txt}\n"
                "**Kredensial Heroku tidak valid untuk deploy Zycho USERBOT dyno.**"
            )
            return repo.__del__()
        try:
            from Zycho.modules.sql_helper.globals import addgvar, delgvar
        ...
        await event.edit(
            "`Zycho USERBOT Berhasil Di Deploy! Userbot bisa di gunakan kembali.`"
        )
...
async def update(event, repo, ups_rem, ac_br):
    try:
        ...
        repo.git.reset("--hard", "FETCH_HEAD")
    await event.edit("`Zycho USERBOT Berhasil Diupdate! Userbot bisa di Gunakan Lagi.`")
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"update( now| deploy|$)"))
async def upstream(event):
    ...
    if conf == "deploy":
        await event.edit("`[HEROKU]: Update Deploy Zycho USERBOT Sedang Dalam Proses...`")
        await deploy(event, repo, ups_rem, ac_br, txt)
        return

    if changelog == "" and not force_update:
        await event.edit("**✥ Zycho USERBOT Sudah Versi Terbaru**")
...
CMD_HELP.update(
    {
        ...
        \n\n  •  **Syntax :** `{cmd}update`\
        \n  •  **Function : **Untuk Melihat Pembaruan Terbaru Zycho USERBOT.\
        \n\n  •  **Syntax :** `{cmd}update deploy`\
        \n  •  **Function : **Untuk MengUpdate Fitur Terbaru Dari Zycho USERBOT.\
    "
    }
)
