...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"notes$"))
async def notes_active(svd):
    """For .notes command, list all of the notes saved in a chat."""
    try:
        from Zycho.modules.sql_helper.notes_sql import get_notes
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"clear (\w*)"))
async def remove_notes(clr):
    """For .clear command, clear note with the given name."""
    try:
        from Zycho.modules.sql_helper.notes_sql import rm_note
    except AttributeError:
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"save (\w*)"))
async def add_note(fltr):
    """For .save command, saves notes in a chat."""
    try:
        from Zycho.modules.sql_helper.notes_sql import add_note
    except AttributeError:
...
async def incom_note(getnt):
    ...
    try:
        if not (await getnt.get_sender()).bot:
            try:
                from Zycho.modules.sql_helper.notes_sql import get_note
            except AttributeError:
                return
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"rmbotnotes (.*)"))
