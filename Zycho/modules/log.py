...
from Zycho import CMD_HELP, LOGS, bot
from Zycho.events import Zycho_cmd
from Zycho.modules.sql_helper import no_log_pms_sql
from Zycho.modules.sql_helper.globals import addgvar, gvarstatus
from Zycho.modules.vcplugin import vcmention
from Zycho.utils import _format, edit_delete
from Zycho.utils.logger import logging
from Zycho.utils.tools import media_type
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"save(?: |$)(.*)"))
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"log$"))
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"nolog$"))
async def set_no_log_p_m(event):
    if BOTLOG_CHATID != -100:
        chat = await event.get_chat()
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"pmlog (on|off)$"))
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"gruplog (on|off)$"))
