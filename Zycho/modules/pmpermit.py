...
from Zycho import ALIVE_NAME, BOTLOG, BOTLOG_CHATID
from Zycho import CMD_HANDLER as cmd
from Zycho import CMD_HELP, COUNT_PM, LASTMSG, LOGS, PM_AUTO_BAN, PM_LIMIT, bot
...
async def permitpm(event):
    ...
    if (
        ...
        try:
            ...
            from Zycho.modules.sql_helper.pm_permit_sql import is_approved
...
async def auto_accept(event):
    ...
    if (
        ...
        try:
            from Zycho.modules.sql_helper.globals import gvarstatus
            from Zycho.modules.sql_helper.pm_permit_sql import approve, is_approved
        except AttributeError:
            return
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"notifoff$"))
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"notifon$"))
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"(?:setuju|ok)\s?(.)?"))
async def approvepm(apprvpm):
    """For .ok command, give someone the permissions to PM you."""
    try:
        from Zycho.modules.sql_helper.globals import gvarstatus
        from Zycho.modules.sql_helper.pm_permit_sql import approve
    except AttributeError:
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"(?:tolak|nopm)\s?(.)?"))
async def disapprovepm(disapprvpm):
    try:
        from Zycho.modules.sql_helper.pm_permit_sql import dissprove
    except BaseException:
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"block$"))
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"unblock$"))
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"(set|get|reset) pm_msg(?: |$)(\w*)"))
async def add_pmsg(cust_msg):
    ...
    try:
        import Zycho.modules.sql_helper.globals as sql
    except AttributeError:
