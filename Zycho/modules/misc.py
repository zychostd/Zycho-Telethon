...
# FROM Zycho USERBOT <https://github.com/zychostd/Zycho-Userbot>
...
from Zycho import ALIVE_NAME, BOT_VER, BOTLOG, BOTLOG_CHATID
from Zycho import CMD_HANDLER as cmd
from Zycho import CMD_HELP, HEROKU_API_KEY, HEROKU_APP_NAME, UPSTREAM_REPO_BRANCH, bot
from Zycho.events import Zycho_cmd
from Zycho.utils import time_formatter
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"sleep ([0-9]+)$"))
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"shutdown$"))
async def killdabot(event):
    ...
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            "**#SHUTDOWN** \n"
            "**Zycho USERBOT** telah di matikan!\nJika ingin menghidupkan kembali silahkan buka heroku",
        )
    await event.edit("`Zycho USERBOT Berhasil di matikan!`")
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"restart$"))
async def killdabot(event):
    await event.edit("**Zycho USERBOT Berhasil di Restart**")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID, "#RESTART \n" "**Zycho USERBOT Berhasil Di Restart**"
        )
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"readme$"))
async def reedme(e):
    await e.edit(
        "**Berikut sesuatu untuk kamu baca:**\n"
        "\n• [Userbot Repo](https://github.com/zychostd/Zycho-Userbot/blob/Zycho-Ubot/README.md)"
        "\n• [Video Tutorial](https://youtu.be/tTDaPKsGC1I)"
        "\n• [List Variabel Heroku untuk Zycho USERBOT](https://telegra.ph/List-Variabel-Heroku-untuk-Zycho USERBOT-09-22)"
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"repeat (.*)"))
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"raw$"))
async def raw(event):
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"reverse(?: |$)(\d*)"))
...
@bot.on(Zycho_cmd(outgoing=True, pattern=r"send (.*)"))
...
CMD_HELP.update(
    {
        ...
        \n\n  •  **Syntax :** `{cmd}sleep`\
        \n  •  **Function : **Biarkan Zycho USERBOT tidur selama beberapa detik \
    "
    }
)
...
CMD_HELP.update(
    {
        "repo": f"**Plugin : **`Repository Zycho USERBOT`\
        \n\n  •  **Syntax :** `{cmd}repo`\
        \n  •  **Function : **Menampilan link Repository Zycho USERBOT\
        \n\n  •  **Syntax :** `{cmd}string`\
        \n  •  **Function : **Menampilan link String Zycho USERBOT\
    "
    }
)
...
CMD_HELP.update(
    {
        "restart": f"**Plugin : **`Restart Zycho USERBOT`\
        \n\n  •  **Syntax :** `{cmd}restart`\
